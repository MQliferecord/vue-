<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span v-show="catogoryView.category1Name">{{ category1Name }}</span>
        <span v-show="catogoryView.category2Name">{{ category2Name }}</span>
        <span v-show="catogoryView.category3Name">{{ category3Name }}</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        <!-- 左侧放大镜区域 -->
...
export default {
  ...
  methods:{
    ...
    async addShopCart(){
      try{
        await this.$store.dispatch('addorUpdateShopCart',{skuId:this.$router.skuId,skuNum:this.$router.skuNum});
        sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo));
        this.$router.push({name:"addcartsuccess",query:{skuNum:this.$router.skuNum}});
      }catch(error){
        alert(error.message);
      }
    }
